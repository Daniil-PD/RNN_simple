...
class LongShortTermMemory(torch.nn.Module):
    def __init__(self, input_size, hidden_size):
        ...
        self.gru = torch.nn.LSTM(input_size, hidden_size, batch_first=True)
    ...
    def init_hidden(self, batch_size = 1, batched = False, device = torch.device('cpu')):
        ...
        if batched:
            return (torch.zeros(1, batch_size, self.gru.hidden_size).to(device),
                    torch.zeros(1, batch_size, self.gru.hidden_size).to(device))
        else:
            return (torch.zeros(1, self.gru.hidden_size).to(device),
                    torch.zeros(1, self.gru.hidden_size).to(device))
